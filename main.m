...
static NSArray *CLRecords(void) { return [[NSUserDefaults standardUserDefaults] objectForKey:CLRecordsKey] ?: @[]; }
static void CLSaveRecords(NSArray *r) { [[NSUserDefaults standardUserDefaults] setObject:r forKey:CLRecordsKey]; [[NSUserDefaults standardUserDefaults] synchronize]; }
static double CLNumber(id value) { return [value respondsToSelector:@selector(doubleValue)] ? [value doubleValue] : 0; }
static double CLRecordTotal(NSDictionary *record) { double t=0; for (NSString *n in CLItemNames()) { NSDictionary *v=record[@"items"][n]; t += CLNumber(v[@"price"])*CLNumber(v[@"quantity"]); } return t; }
static NSString *CLMoney(double v) { return [NSString stringWithFormat:@"$ %.0f", v]; }
static NSString *CLText(id value) { return [value isKindOfClass:NSString.class] ? (NSString *)value : @""; }
...
- (void)recalculate:(id)sender { double t=0; for (CLItemFields *f in self.fields) t+=f.price.text.doubleValue*f.quantity.text.doubleValue; self.totalLabel.text=CLMoney(t); }
- (NSDictionary *)currentRecord { NSMutableDictionary *items=[NSMutableDictionary dictionary]; for (CLItemFields *f in self.fields) items[f.name]=@{@"price":f.price.text?:@"",@"quantity":f.quantity.text?:@""}; return @{@"date":CLDateString(self.datePicker.date),@"location":self.location.text?:@"",@"items":items}; }
...
- (void)reload { self.records=[[CLRecords() sortedArrayUsingComparator:^NSComparisonResult(NSDictionary *a,NSDictionary *b){return [b[@"date"] compare:a[@"date"]];}] copy] ?: @[]; double total=0; for(NSDictionary *r in self.records) total+=CLRecordTotal(r); self.summaryLabel.text=self.records.count?[NSString stringWithFormat:@"累計營業額 %@\n共記錄 %lu 天",CLMoney(total),(unsigned long)self.records.count]:@"目前還沒有歷史紀錄\n先到「記帳」頁保存第一筆資料"; [self.tableView reloadData]; }
- (NSInteger)tableView:(UITableView *)tableView numberOfRowsInSection:(NSInteger)section { return self.records.count; }
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath { UITableViewCell *c=[tableView dequeueReusableCellWithIdentifier:@"record"] ?: [[UITableViewCell alloc] initWithStyle:UITableViewCellStyleSubtitle reuseIdentifier:@"record"]; NSDictionary *r=self.records[indexPath.row]; c.backgroundColor=UIColor.whiteColor; c.layer.cornerRadius=16; c.textLabel.font=[UIFont systemFontOfSize:17 weight:UIFontWeightBold]; c.textLabel.textColor=CLInk(); c.detailTextLabel.font=[UIFont systemFontOfSize:13]; c.detailTextLabel.textColor=[UIColor colorWithWhite:.5 alpha:1]; c.textLabel.text=CLMoney(CLRecordTotal(r)); NSString *location=CLText(r[@"location"]); c.detailTextLabel.text=[NSString stringWithFormat:@"%@   %@",CLDateDisplay(r[@"date"]),location.length?location:@"未填地點"]; c.accessoryType=UITableViewCellAccessoryDisclosureIndicator; return c; }
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath { NSDictionary *r=self.records[indexPath.row]; NSString *location=CLText(r[@"location"]); NSMutableString *m=[NSMutableString stringWithFormat:@"%@\n%@\n\n",CLDateDisplay(r[@"date"]),location.length?location:@"未填地點"]; for (NSString *n in CLItemNames()) { NSDictionary *v=r[@"items"][n]; NSString *price=CLText(v[@"price"]); NSString *quantity=CLText(v[@"quantity"]); [m appendFormat:@"%@   %@ × %@ = %@\n",n,price.length?price:@"0",quantity.length?quantity:@"0",CLMoney(CLNumber(v[@"price"])*CLNumber(v[@"quantity"]))]; } [m appendFormat:@"\n總額：%@",CLMoney(CLRecordTotal(r))]; UIAlertController *a=[UIAlertController alertControllerWithTitle:@"銷售明細" message:m preferredStyle:UIAlertControllerStyleAlert]; [a addAction:[UIAlertAction actionWithTitle:@"關閉" style:UIAlertActionStyleCancel handler:nil]]; [self presentViewController:a animated:YES completion:nil]; [tableView deselectRowAtIndexPath:indexPath animated:YES]; }
- (void)tableView:(UITableView *)tableView commitEditingStyle:(UITableViewCellEditingStyle)style forRowAtIndexPath:(NSIndexPath *)indexPath { if(style!=UITableViewCellEditingStyleDelete)return; NSDictionary *r=self.records[indexPath.row]; UIAlertController *a=[UIAlertController alertControllerWithTitle:@"刪除這筆紀錄？" message:[NSString stringWithFormat:@"%@ %@",CLDateDisplay(r[@"date"]),CLMoney(CLRecordTotal(r))] preferredStyle:UIAlertControllerStyleAlert]; [a addAction:[UIAlertAction actionWithTitle:@"取消" style:UIAlertActionStyleCancel handler:nil]]; [a addAction:[UIAlertAction actionWithTitle:@"刪除" style:UIAlertActionStyleDestructive handler:^(UIAlertAction *action){ NSMutableArray *all=[CLRecords() mutableCopy]; for(NSInteger i=all.count-1;i>=0;i--)if([all[i][@"date"] isEqual:r[@"date"]])[all removeObjectAtIndex:i]; CLSaveRecords(all); [self reload]; }]]; [self presentViewController:a animated:YES completion:nil]; }
